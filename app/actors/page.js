...
export default async function Actors() {
  try {
    // Fetching data from the 'person/popular' endpoint
            ...
            >
              {actors.map((actor) => (
                ...
                >
                  ...
                  <p className="text-gray-600">Popularity: {actor.popularity.toFixed(1)}</p>
                </div>
              ))}
            </div>
          </div>
    ...
    return (
      <div className="container mx-auto p-4">
        <h1 className="text-2xl font-bold mb-4">Popular Actors</h1>
        <p className="text-red-500">Failed to load actors. Please try again later.</p>
      </div>
    );
  }
}
